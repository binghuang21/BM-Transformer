...
parser.add_argument("--mode",default="inference",type=str,choices=["train", "inference"])    # fixme train
...
parser.add_argument("--data_parallel", default= 0, type=int)    # fixme train 1

parser.add_argument("--exp_name", default='Time-my', type=str)   # fixme train name
parser.add_argument("--load_ckt", default="Time-my", type=str)   #pre-train model   # fixme train none
parser.add_argument("--load_ckt_loss", default="8", type=str)     #pre-train model   # fixme train, inf  default="8",
parser.add_argument("--path_train_data", default='bmt', type=str)
parser.add_argument("--data_root", default='/mnt/guided/plan2data/corpus-time-my', type=str)
parser.add_argument("--load_dict", default="dictionary.pkl", type=str)
parser.add_argument("--init_lr", default= 0.00001, type=float)
# inference config
parser.add_argument("--videos", default='/mnt/guided/pre_train/video2npz/inference-add/', type=str)
parser.add_argument("--num_songs", default=10, type=int)
...
if MODE == 'train':
    path_exp = '/mnt/guided/plan2data/exp/' + args.exp_name
...
if args.load_ckt == 'none':
    ...
    info_load_model = (
        ...
        '/mnt/guided/plan2data/exp/' + args.load_ckt,
        # loss
        args.load_ckt_loss                               
        )
...
if args.out_dir == 'none':
    path_gendir = os.path.join('/mnt/guided/plan2data/exp/' + args.load_ckt, 'gen_midis', 'loss_'+ args.load_ckt_loss)    # ?????
else:
    ...
    for epoch in range(n_epoch):
        ...
        for bidx, (batch_x, batch_y, batch_mask)  in enumerate(train_loader): # num_batch 
            ...
            if args.data_parallel > 0:
                ...
                loss = (losses[0] + losses[1] + losses[2] + losses[3] + losses[4] + losses[5] + losses[6] + losses[7] + losses[8] + losses[9] + losses[10]) / 11
        ...
        each_loss_str = '{:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}, {:04f}\r'.format(
                  acc_losses[0], acc_losses[1], acc_losses[2], acc_losses[3], acc_losses[4], acc_losses[5], acc_losses[6], acc_losses[7], acc_losses[8], acc_losses[9], acc_losses[10])
...
def generate():
    ...
    for video in filelist:
        ...
        name = 'loss_' + str(loss)
        path_saved_ckpt = os.path.join(path_ckpt, name + '_params.pt')
        video_emo_tag = int(video.split('/')[-1].split('_')[0])
        video_name = video.split('/')[-1].split('_')[1].split('.')[0]
        ...
        while sidx < num_songs:
            try:
                ...
                if n_token == 11:
                    path_outfile = os.path.join(path_gendir, 'emo_{}_{}_{}_{}'.format( str(video_emo_tag), video_name, sidx, get_random_string(10)))
                    res, _ = net.inference_from_scratch(dictionary, video_emo_tag, n_token, video_npz=video_npz)
